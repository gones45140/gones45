...
if(window.location.hash.indexOf('#simu=')>=0) setTimeout(chargerSimuDepuisUrl, 800);
...
function partagerSimu() {
  ...
  modal.innerHTML = '<div style="background:var(--s1);border:1px solid var(--b2);border-radius:16px;padding:20px;max-width:420px;width:100%;max-height:80vh;overflow-y:auto;">'
    ...
    +'</div>'
    +'<textarea id="simu-share-url" readonly style="width:100%;height:70px;background:rgba(255,255,255,.04);border:1px solid var(--b2);border-radius:8px;color:var(--t3);font-size:10px;padding:8px;resize:none;margin-bottom:12px;box-sizing:border-box;">'+url+'</textarea>'
    +'<div style="display:flex;flex-direction:column;gap:8px;">'
    +'<a href="'+waUrl+'" target="_blank" style="display:flex;align-items:center;justify-content:center;gap:10px;padding:14px;background:#25d366;border-radius:10px;color:#fff;font-size:14px;font-weight:800;text-decoration:none;">📱 Envoyer sur WhatsApp</a>'
...
function chargerSimuDepuisUrl() {
  ...
  try {
    ...
    // Sauvegarder dans les simis REÇUES — pas écraser la simu active
    ...
    if(!existe) {
      ...
      if(recues.length > 20) recues = recues.slice(0,20); // max 20 simis
